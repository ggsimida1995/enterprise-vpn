...
for target in darwin/arm64 darwin/amd64 windows/amd64; do
	case "$target" in
		darwin/arm64) upstream=macos-aarch64; output=easytier-core-darwin-arm64 ;;
		darwin/amd64) upstream=macos-x86_64; output=easytier-core-darwin-amd64 ;;
		windows/amd64) upstream=windows-x86_64; output=easytier-core-windows-amd64 ;;
	esac
